...
def gleam(im: Image, gamma_coeff: float=2.2) -> Image:
    ...
    im = np.mean(im, axis=2)  # 在第三个维度上求均值
    return np.expand_dims(im, axis=2)  # 增加一个维度
...
def open_grayscale(path: str) -> Image:
    ...
    :return: The grayscale image. 最终返回的图像的维度为3
    """
...
def get_edges(im: Image, lo: float=175, hi: float=220, window: int=3) -> Image:
    ...
    使用Canny检测边缘, 边缘点的像素值为255 非边缘点的像素值为0
...
def apply_swt(im: Image, edges: Image, gradients: Gradients, dark_on_bright: bool=True) -> Image:
    ...
    :param im: The image 原始图像
    :param edges: The edges of the image. 图像的边缘图
    :param gradients: The gradients of the image. 图像的梯度
    :param dark_on_bright: Enables dark-on-bright text detection. 字体是否为黑色
    :return: The transformed image.
    """
    # Prepare the output map. 最终的结果，每个像素点的宽度
    ...
    # Find a pixel that lies on an edge. 寻找每个像素点对应的像素点
    height, width = im.shape[0:2]
    for y in range(height):
        for x in range(width):
            # Edges are either 0. or 1. 判断是否当前元素是否为边缘上一点
            if edges[y, x] < .5:
                continue
            # ray数组包含了当前梯度射线所经过的点
    ...
    # 为了应对特殊情况，如'L'，此时会将射线上所有像素的宽度取均值
    for ray in rays:
...
def swt_process_pixel(pos: Position, edges: Image, directions: Gradients, out: Image, dark_on_bright: bool=True) -> Optional[Ray]:
    ...
    while True:
        ...
        for p in ray:
            out[p.y, p.x] = min(stroke_width, out[p.y, p.x])  # 这条射线上所有点的宽度都会被计算
        return ray
